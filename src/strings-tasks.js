...
function getFirstChar(value) {
  return value === undefined || null ? '' : value.charAt(0);
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < myArr.length; i += 1) {
    if (myArr[i].length > longestWord) {
      longestWord = myArr[i].length;
      answer = myArr[i];
    }
  }
  return answer;
...
function reverseWords(str) {
  ...
  const reverseArr = myArr.map((item) => item.split('').reverse().join(''));
